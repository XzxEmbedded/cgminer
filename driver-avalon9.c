...
char *set_avalon9_asic_otp(char *arg)
{
	int val, ret;

	ret = sscanf(arg, "%d", &val);
	if (ret < 1)
		return "No value passed to avalon9-cinfo-asic";

	if (val < 0 || val > (AVA9_DEFAULT_ASIC_MAX - 1))
		return "Invalid value passed to avalon9-cinfo-asic";

	opt_avalon9_asic_otp = val;

	opt_avalon9_cycle_hit_flag = 0;
...
static int decode_pkg(struct cgpu_info *avalon9, struct avalon9_ret *ar, int modular_id)
{
	...
	case AVA9_P_STATUS:
		...
		break;
	case AVA9_P_STATUS_OTP:
		if (opt_avalon9_cycle_hit_flag)
			break;

		applog(LOG_DEBUG, "%s-%d-%d: AVA9_P_STATUS_OTP", avalon9->drv->name, avalon9->device_id, modular_id);

		/* ASIC reading cycle limit hit */
		if (ar->data[AVA9_OTP_INDEX_CYCLE_HIT]) {
			applog(LOG_DEBUG, "%s-%d-%d: AVA9_P_STATUS_OTP, OTP read cycle hit!", avalon9->drv->name, avalon9->device_id, modular_id);
			opt_avalon9_cycle_hit_flag = 1;
			break;
		}
		...
		switch (ar->data[AVA9_OTP_INDEX_READ_STEP]) {
		case 0:
			memcpy(info->otp_info[modular_id][miner_id] + AVA9_OTP_INFO_LOTIDCRC_OFFSET, ar->data + AVA9_OTP_INFO_LOTIDCRC_OFFSET, 4);
			break;
		case 1:
			memcpy(info->otp_info[modular_id][miner_id] + AVA9_OTP_INFO_LOTIDCRC_OFFSET + 4, ar->data + AVA9_OTP_INFO_LOTIDCRC_OFFSET + 4, 2);
			break;
		case 2:
			memcpy(info->otp_info[modular_id][miner_id] + AVA9_OTP_INFO_LOTID_OFFSET, ar->data + AVA9_OTP_INFO_LOTID_OFFSET, 4);
			break;
		case 3:
			memcpy(info->otp_info[modular_id][miner_id] + AVA9_OTP_INFO_LOTID_OFFSET + 4, ar->data + AVA9_OTP_INFO_LOTID_OFFSET + 4, 4);
			break;
		case 4:
			memcpy(info->otp_info[modular_id][miner_id] + AVA9_OTP_INFO_LOTID_OFFSET + 8, ar->data + AVA9_OTP_INFO_LOTID_OFFSET + 8, 4);
			break;
		case 5:
			memcpy(info->otp_info[modular_id][miner_id] + AVA9_OTP_INFO_LOTID_OFFSET + 12, ar->data + AVA9_OTP_INFO_LOTID_OFFSET + 12, 4);
			break;
		case 6:
			memcpy(info->otp_info[modular_id][miner_id] + AVA9_OTP_INFO_LOTID_OFFSET + 16, ar->data + AVA9_OTP_INFO_LOTID_OFFSET + 16, 4);
			break;
		default:
			break;
		}

		/* get the data behind AVA9_OTP_INDEX_READ_STEP for later displaying use */
		memcpy(info->otp_info[modular_id][miner_id] + AVA9_OTP_INDEX_READ_STEP, ar->data + AVA9_OTP_INDEX_READ_STEP, 4);
	...
	case AVA9_P_STATUS_PLL:
		...
		for (i = 0; i < AVA9_DEFAULT_PLL_CNT; i++) {
			...
			memcpy(&tmp, ar->data + AVA9_DEFAULT_PLL_CNT * 4 + i * 4, 4);
			tmp = be32toh(tmp);
			if (tmp)
	...
	case AVA9_P_STATUS_ASIC:
		{
			...
			for (i = 0; i < AVA9_DEFAULT_PLL_CNT; i++) {
				memcpy(&freq, ar->data + 8 + AVA9_DEFAULT_PLL_CNT + i * 2, 2);
...
static struct cgpu_info *avalon9_iic_detect(void)
{
	...
	applog(LOG_INFO, "%s-%d: Found at %s", avalon9->drv->name, avalon9->device_id, I2C_BUS);
...
static void detect_modules(struct cgpu_info *avalon9)
{
	...
	for (i = 1; i < AVA9_DEFAULT_MODULARS + 1; i++) {
		...
		for (j = 0; j < info->miner_count[i]; j++) {
			...
			if (AVA9_INVALID_ASIC_OTP == opt_avalon9_asic_otp)
				info->set_asic_otp[i][j] = 0; /* default asic: 0 */
			else
				info->set_asic_otp[i][j] = opt_avalon9_asic_otp;
		}
...
static void avalon9_set_asic_otp(struct cgpu_info *avalon9, int addr, unsigned int asic[])
{
	struct avalon9_info *info = avalon9->device_data;
	struct avalon9_pkg send_pkg;
	...
	for (i = 0; i < info->miner_count[addr]; i++) {
		if (asic[i] < 0)
			asic[i] = 0;
		else if (asic[i] > (AVA9_DEFAULT_ASIC_MAX -1))
			asic[i] = AVA9_DEFAULT_ASIC_MAX - 1;
		tmp = be32toh(asic[i]);
		memcpy(send_pkg.data + i * 4, &tmp, 4);
	}
	applog(LOG_DEBUG, "%s-%d-%d: avalon9 set asic for otp reading %d, (%d-%d)",
			avalon9->drv->name, avalon9->device_id, addr,
			i, asic[0], asic[info->miner_count[addr] - 1]);

	/* Package the data */
	avalon9_init_pkg(&send_pkg, AVA9_P_SET_ASIC_OTP, 1, 1);
	if (addr == AVA9_MODULE_BROADCAST)
		avalon9_send_bc_pkgs(avalon9, &send_pkg);
	else
		avalon9_iic_xfer_pkg(avalon9, addr, &send_pkg, NULL);
...
static void avalon9_set_ss_param(struct cgpu_info *avalon9, int addr)
{
	...
	tmp = be32toh(opt_avalon9_th_add);
	...
	for (i = 1; i < AVA9_DEFAULT_MODULARS; i++) {
		...
		switch (info->freq_mode[i]) {
			...
			case AVA9_FREQ_PLLADJ_MODE:
				...
				/* AVA9_DEFAULT_SMARTSPEED_MODE1: auto speed by A3210 chips */
				break;
			default:
...
static char *avalon9_set_device(struct cgpu_info *avalon9, char *option, char *setting, char *replybuf)
{
	...
	if (strcasecmp(option, "led") == 0) {
		...
		if (val < 1 || val >= AVA9_DEFAULT_MODULARS) {
			sprintf(replybuf, "invalid module_id: %d, valid range 1-%d", val, AVA9_DEFAULT_MODULARS);
			return replybuf;
		}
